...
export type Phase =
  ...
  | "JAIL_DECISION"
  /**
   * UX-фаза «Вы арестованы! Отправляйтесь в тюрьму!».
   *
   * Сервер показывает информационное модальное окно `JailNoticeModal`
   * (по аналогии с `CARD_REVEAL`), и ТОЛЬКО после подтверждения
   * игрока/бота (`CONFIRM_JAIL_NOTICE`) запускается анимация фишки
   * к клетке 10 (JAIL) и далее — стандартный flow через
   * `MOVE_ANIMATION` → `RESOLVING_LANDING` → `JAIL_DECISION`.
   *
   * Используется в 2 случаях:
   *  1) Попадание на клетку 30 (GOTO_JAIL) — это НЕ карточка Шанс/Казна,
   *     а отдельное правило Монополии.
   *  2) Три дубля подряд — правило «обрыва серии дублей».
   */
  | "JAIL_NOTICE"
...
export interface GameState {
   ...
   * (move to=10, goto-jail) и в handleDiceAnimation (правило 3 дублей).
   * После отправки в тюрьму — сбрасывается.
   */
  pendingJailFromCard?: boolean;
  /**
   * Причина попадания в тюрьму через `pendingJailFromCard`. Используется
   * только в `handleResolvingLanding` для логирования в LogPanel.
   * Возможные значения:
   *  - `"card"`   — попадание через карточку (move to=10, goto-jail outcome);
   *  - `"double"` — попадание через 3 дубля подряд (правило Монополии).
   *
   * Необязательное поле. Если не задано — считается `"card"`
   * (обратная совместимость со старыми снапшотами).
   */
  pendingJailReason?: "card" | "double";
  /**
   * Контекст активного информационного модального окна «Вы арестованы!
   * Отправляйтесь в тюрьму!» (фаза `JAIL_NOTICE`).
   *
   * Содержит ID игрока, для которого показано окно, и причину ареста:
   *  - `"cell"`   — попадание на клетку 30 (GOTO_JAIL) броском кубиков;
   *  - `"card"`   — карточка Шанс/Казна «Отправляйтесь в тюрьму» (на
   *                 данный момент не используется напрямую — такие карты
   *                 идут через `CARD_REVEAL` → `CARD_EFFECT` →
   *                 `applyCardEffectAndAdvance` → `MOVE_ANIMATION`,
   *                 но при рефакторинге поле сохранено для совместимости);
   *  - `"double"` — три дубля подряд (правило Монополии).
   *
   * Сбрасывается (становится `undefined`) в `handleJailNotice` после
   * `CONFIRM_JAIL_NOTICE` — окно закрыто, начинается анимация фишки.
   */
  jailNotice?: {
    playerId: string;
